...
import { getForums, createForum, forumShort } from '../../api/forum';
import { RootState } from '../../store/store';
import { forumShort as forumObject } from '../../api/forum';

const Forum = () => {
    const [newForumName, setNewForumName] = useState('');
    const [forums, setForums] = useState<forumShort[]>([]);
    ...
    async function createForumBtn(): Promise<void> {
        if (newForumName.length > 100) return;

        const newForum = await createForum(newForumName);
                        ...
                        {forums.map((forumEl: forumObject) => (
                            <li key={forumEl.id} className={styles['forum__thread']} onClick={() => navigate(`./${forumEl.id}`)}>
                                ...
                                <div className={styles['forum__thread__info']}>
                                    ...
                                    <p className={styles['forum__thread__info__paragraph']}>{forumEl.publishedDateTime.split('T')[0]}</p>
                                </div>
                            </li>
                        ))}
                    </ul>
